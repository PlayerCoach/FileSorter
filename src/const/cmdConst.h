...
const std::string INPUT_FILE_CMD = "input";
  ...
  const::std::string menu = "* " + ADD_CMD +" [n] [" + RANDOM_FLAG + "/" + SPECIFIED_FLAG + "] [" + EXPLICIT_FLAG + "] | Add n random/specified records to file, -e shows them *\n" 
    ...
    "* " + SORT_CMD + " [ " + EXPLICIT_FLAG + "/" + HIDDEN_FLAG + "] | Sorts file [" + HIDDEN_FLAG + "] prints only final result [" + EXPLICIT_FLAG+ "] + {N}(optional - show N first records from every phase) shows file after every phase * \n" 
